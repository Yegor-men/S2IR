...
class SIIR(nn.Module):
	...
	def forward(
		...
		#
		# semantic_map = semantic_map + col_hint2 + pos_hint2
		semantic_map = self.semantic_norm(semantic_map)
		...
		if pos_cond is not None:
			...
			cross_scalar = self.cross_scalar.view(1, self.d_channels, 1, 1)
			eps_pos = eps_pos + pos_cross_delta * cross_scalar
			for i, block in enumerate(self.dec_blocks):
		...
		if neg_cond is not None:
			...
			cross_scalar = self.cross_scalar.view(1, self.d_channels, 1, 1)
			eps_neg = eps_neg + neg_cross_delta * cross_scalar
			for i, block in enumerate(self.dec_blocks):
